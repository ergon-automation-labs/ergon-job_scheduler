pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-job_scheduler"
...
EOF
            ...
            /opt/bot_army/scripts/nats_publish.sh ops.deploy.job_scheduler "$PAYLOAD" || echo "⚠️  NATS publish failed (non-blocking)"
            exit 1
          fi
...
EOF
          ...
          $RELEASE_BIN eval 'BotArmyJobScheduler.Release.migrate()' && {
            ...
            VERSION=$(awk '{print $2}' "$RELEASE_DIR/current/job_scheduler/releases/start_erl.data" 2>/dev/null || echo "unknown")
            PAYLOAD=$(cat <<EOF
